...
TREE_TAGGER_CMD = "/Users/mac/Desktop/vscode/gromov/tree_tagger/cmd/tree-tagger-english"

def tag_with_treetagger(text):
    """
...
def sentence_to_conllx(sentence):
    """
    Преобразует предложение в формат CoNLL-X с POS-тегами.
    HEAD = 0, DEPREL = _
    """
    tagged_tokens = tag_with_treetagger(sentence)
    lines = []
...
def write_conllx_file(sentence, filename="input.conll"):
    """
    Сохраняет предложение в файл в формате CoNLL-X с POS-тегами.
    """
    conll_data = sentence_to_conllx(sentence)
...
if __name__ == "__main__":
    ...
    file_path = write_conllx_file(test_sentence)
